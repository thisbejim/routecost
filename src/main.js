import { calculatePlan, clonePlan, convertPlanUnit, makeDefaultPlan, } from './model';
import './styles.css';
const STORAGE_KEY = 'routecost-plan-v1';
const SHARE_PREFIX = 'share=';
const BASE_URL = 'https://thisbejim.github.io/routecost/';
const CURRENCY_CODES = ['AUD', 'USD', 'CAD', 'NZD', 'EUR', 'GBP', 'JPY'];
let state = makeDefaultPlan();
let isExample = true;
let nextLegId = 3;
const appRoot = document.querySelector('#app');
if (!appRoot)
    throw new Error('RouteCost app root is missing.');
const app = appRoot;
...
function validCurrency(value) {
    return CURRENCY_CODES.includes(value) ? String(value) : 'AUD';
}
...
function energyLabel(mode = state.vehicleMode) {
    if (mode === 'ev')
        return 'kWh';
    return state.unitSystem === 'metric' ? 'L' : 'gal';
}
function energyInputLabel(mode) {
    if (mode === 'ev')
        return state.unitSystem === 'metric' ? 'Efficiency (kWh / 100 km)' : 'Efficiency (miles / kWh)';
    return state.unitSystem === 'metric' ? 'Efficiency (L / 100 km)' : 'Fuel economy (miles / gallon)';
}
function priceInputLabel(mode) {
    if (mode === 'ev')
        return 'Charging price (per kWh)';
    return state.unitSystem === 'metric' ? 'Fuel price (per litre)' : 'Fuel price (per gallon)';
}
function input(field, label, value, options = {}) {
...
function renderForm() {
    ...
    if (!form)
        return;
    const alternateMode = state.vehicleMode === 'gas' ? 'ev' : 'gas';
...
function resultCard(result, mode, isCurrent = false) {
...
function summaryText(result, mode = state.vehicleMode) {
...
function getByPath(path) {
    if (path === 'roundTrip' || path === 'compareVehicles')
        return state[path];
    if (path === 'currency')
        return state.currency;
    if (path === 'bufferPercent' || path === 'tripDays' || path === 'people' || path === 'tolls' || path === 'parking' || path === 'lodgingNights' || path === 'lodgingPerNight' || path === 'foodPerPersonPerDay' || path === 'activities')
        return state[path];
    if (path === 'gas.efficiency' || path === 'gas.price')
        return state.gas[path.split('.')[1]];
    if (path === 'ev.efficiency' || path === 'ev.price')
        return state.ev[path.split('.')[1]];
    return '';
}
function updateField(field, value) {
    if (field.startsWith('leg:')) {
        ...
        if (!leg)
            return;
        if (property === 'name')
            leg.name = String(value).slice(0, 80);
        if (property === 'distance')
            leg.distance = Math.max(0, numberValue(value));
        return;
    }
    if (field === 'gas.efficiency' || field === 'gas.price') {
        state.gas[field.split('.')[1]] = Math.max(0, numberValue(value));
        return;
    }
    if (field === 'ev.efficiency' || field === 'ev.price') {
        state.ev[field.split('.')[1]] = Math.max(0, numberValue(value));
        return;
    }
    if (field === 'currency') {
    ...
    if (field === 'tripDays' || field === 'people' || field === 'lodgingNights') {
        ...
        if (field === 'people')
            state.people = Math.max(1, state.people);
        return;
    }
...
function handleFormValue(target) {
    ...
    markChanged();
    const routeTotal = document.querySelector('#route-total-note');
    if (routeTotal) {
    ...
    if (field === 'bufferPercent') {
        const output = document.querySelector('#buffer-value');
        if (output)
            output.value = `${Math.round(state.bufferPercent)}%`;
...
function handleAction(button) {
    ...
    if (action === 'add-leg') {
        ...
        renderForm();
        renderResults();
        window.requestAnimationFrame(() => document.querySelector('#legs-list .leg-row:last-child input[type="number"]')?.focus());
        return;
    }
