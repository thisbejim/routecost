...
export type VehicleMode = 'gas' | 'ev';
...
function calculateEnergy(
  ...
  plan: Plan,
  mode: VehicleMode,
): { used: number; cost: number; unit: string } {
...
export function validatePlan(plan: Plan): string[] {
  ...
  if (!distance) errors.push('Add at least one route distance.');
  ...
  if (!Number.isFinite(vehicle.price) || vehicle.price < 0) errors.push('Enter a valid energy price.');
  if (!Number.isFinite(plan.people) || plan.people < 1) errors.push('Use at least one traveller for the split.');
  if (!Number.isFinite(plan.tripDays) || plan.tripDays < 1) errors.push('Use at least one trip day.');
  if (!Number.isFinite(plan.lodgingNights) || plan.lodgingNights < 0) errors.push('Lodging nights cannot be negative.');
...
export function calculatePlan(plan: Plan, mode: VehicleMode = plan.vehicleMode): PlanResult {
...
export function convertPlanUnit(plan: Plan, nextUnit: UnitSystem): Plan {
  if (plan.unitSystem === nextUnit) return clonePlan(plan);
